...
class ResNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_features = False) :
        ...
        out = relu(self.bn1(self.conv1(x)))
        f0=out
        out,f1 = self.layer1(out)  # 64, 32, 32
        f1_act=[self.relu(f) for f in f1]
        out,f2 = self.layer2(out)  # 128, 16, 16
        f2_act=[self.relu(f) for f in f2]
        out,f3 = self.layer3(out)  # 256, 8, 8
        f3_act=[self.relu(f) for f in f3]
        out,f4 = self.layer4(out)  # 512, 4, 4
        f4_act=[self.relu(f) for f in f4]
        out = avg_pool2d(out, out.shape[2]) # 512, 1, 1
        out = out.view(out.size(0), -1)  # 512
        f5=out
        if return_features:
            return [f0]+f1_act+f2_act+f3_act+f4_act,self.linear(out), self.eq_head(out), F.normalize(self.inv_head(out), dim=1), F.normalize(out, dim=1)
